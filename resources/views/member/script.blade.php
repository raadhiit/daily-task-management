...
<script>
    ...
    var leadTime = {!! json_encode($jobList->lead_time) !!};
    var leadTimeHour = leadTime * 3600;
    var autoRun = "{{ Session::get('task_' . Session::get('id_mch')) }}";
    var statusJob = "{{ Session::get('status_' . Session::get('id_mch')) }}";
    ...
    function updateStopAndProdHour() {
        $.ajax({
            url: "{{ route('downtime.submitStop', ['id_ljkh' => $ljkh->id_ljkh]) }}",
            type: "POST",
            dataType: "json",
            data: {
                _token: $('meta[name="csrf-token"]').attr('content')
    ...
    function NPKNULL(id_ljkh, selectedActivity) {
        $.ajax({
            ...
            type: "POST",
            url: "{{ route('member.autoRunNPK', ':id_ljkh') }}".replace(':id_ljkh', id_ljkh),
            data: {
    ...
    function status() {
        ...
        $.get("{{ route('member.showJobMember', ':id_ljkh') }}".replace(':id_ljkh', id), function(data) {
    ...
    function checkNPKActivity() {
        ...
        var inProgressActivities = {!! json_encode($ljkh->where('status', 'In progress')->pluck('activity_name')->toArray()) !!};
    ...
    $(document).ready(function() {
        ...
        $("#anForm button").click(function(e) {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('member.start', ['id_ljkh=> $ljkh->id_ljkh']) }}",
                data: {
                    _token: "{{ csrf_token() }}",
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        ...
                        url: "{{ route('member.stop', ':id_ljkh') }}".replace(':id_ljkh', id),
                        type: 'POST',
                        success: function(response) {
                            Swal.fire({
                                icon: "success",
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                ...
                                type: "POST",
                                url: "{{ route('member.autoRunNPK', ':id_ljkh') }}".replace(':id_ljkh', id),
                                success: function(response) {
                                    Swal.fire({
                                        icon: "info",
                                        title: "Auto Run Dimulai!",
        ...
        $('.EndNpk').on('click', function(e) {
            ...
            Swal.fire({
                ...
                $.ajax({
                    ...
                    url: "{{ route('member.autoRunEnd', ':id_ljkh') }}".replace(':id_ljkh', id),
                    type: 'POST',
                    data: {
                        stop: stopInput
                    },
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        url: "{{ route('member.hold', ':id_ljkh') }}".replace(':id_ljkh', jobId),
                        type: 'POST',
                        dataType: 'json',
                        success: function(response) {
                            Swal.fire({
        ...
        $(document).on('click', '.takeJob', function(e) {
            ...
            $.ajax({
                ...
                url: "{{ route('job.take', ':id_ljkh') }}".replace(':id_ljkh', jobId),
                type: 'POST',
                dataType: 'json',
                success: function(response) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        ...
                        url: "{{ route('member.changeTask', ':id_ljkh') }}".replace(':id_ljkh', id),
                        method: "POST",
                        success: function(response) {
                            stopStopwatch();
                            Swal.fire({
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        ...
                        url: "{{ route('member.endTask', ':id_ljkh') }}".replace(':id_ljkh', id),
                        method: "POST",
                        success: function(response) {
                            stopStopwatch();
                            Swal.fire({
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            $.ajax({
                                ...
                                url: "{{ route('member.shift', ':id_ljkh') }}".replace(':id_ljkh', id),
                                success: function(response) {
                                    setTimeout(function() {
                                        Swal.fire({
                                            icon: "success",
        ...
        $('.settingSelesai').on('click', function(e) {
            ...
            $.ajax({
                ...
                url: "{{ route('member.settingDone') }}",
                type: "POST",
                dataType: 'JSON',
                success: function(response) {
                    $('#notifSetting').modal('hide');
                    var job_id = $this.data('id');
                    $.get("{{ route('member.showJobMember', ':id_ljkh') }}".replace(':id_ljkh', job_id), function(data) {
                    console.log(data);
